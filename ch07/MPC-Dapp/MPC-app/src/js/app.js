App = {
    ...
    populateAddress: function () {
        ...
        new Web3(App.url).eth.getAccounts((err, accounts) => {
            if (!err) {
                ...
                App.web3.eth.getBalance(accounts[0]).then((res) => {
                    jQuery('#sender_balance').text(web3.fromWei(res), "ether");
                });
                App.web3.eth.getBalance(accounts[1]).then((res) => {
                    jQuery('#receiver_balance').text(web3.fromWei(res), "ether");
                });
            } else {
    ...
    handleTransfer: function (amount, signedMessage) {
        ...
        if (App.web3.utils.isHexStrict(signedMessage)) {
            const weiamount = App.web3.utils.toWei(amount, 'ether');
            let amount = App.web3.utils.toHex(weiamount)
            const option = {from: App.receiver};
